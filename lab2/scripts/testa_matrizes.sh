...
executaveisMatrizesMatrizes=(
    "../build/matrix_lib_test_mm_cuda"
)

executaveisEscalares=(
    "../build/matrix_lib_test_scalar_cuda"
)
...
for exec in "${executaveisMatrizesMatrizes[@]}"; do
    for dimensao in "${dimensoesMm[@]}"; do
        for i in $(seq 1 10); do
            ...
            output=$($exec -s "$ESCALAR" -r "$linhas_m1" -c "$colunas_m1" -C "$colunas_m2" -m "$matriz1" -M "$matriz2" -o "$result1" -O "$result2" -t 256 -g 4096)
